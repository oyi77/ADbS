...
## 2026-10-24 - [Pure Bash Random Strings vs /dev/urandom Pipelines]
**Learning:** Generating short random strings using pipelines reading from `/dev/urandom` (e.g., `head | md5sum | cut`) is approximately 30-40x slower than using pure Bash built-ins (string slicing with `$RANDOM`) due to the overhead of spawning multiple subprocesses. For non-cryptographic IDs, the pipeline approach is a significant bottleneck.
**Action:** Prefer pure Bash implementations using `$RANDOM` and string manipulation for generating simple identifiers or random strings where cryptographic security is not required.
